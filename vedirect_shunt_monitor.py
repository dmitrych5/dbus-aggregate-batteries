...
class VeDirectShuntMonitor:
    ...
    def update(self) -> Optional[VeDirectShuntData]:
        ...
        while (frame := self.parser.next_frame()) is not None:
            ...
            consumed_energy_ah = self._parse_int(frame, VeKey.CONSUMED_ENERGY_MAH, 1000.0)
            current_amps = self._parse_int(frame, VeKey.CURRENT_MA, 1000.0)
            soc_percent = self._parse_int(frame, VeKey.SOC_PERMILLE, 10.0)

            if consumed_energy_ah is not None and current_amps is not None and soc_percent is not None:
                self.data = VeDirectShuntData(
                    consumed_energy_ah=consumed_energy_ah, current_amps=current_amps, soc_percent=soc_percent, read_timestamp=time.monotonic()
                )

...
EXPECTED_AUX_VOLTAGES: List[float] = get_list_from_config("DEFAULT", "EXPECTED_AUX_VOLTAGES", float)
EXPECTED_AUX_VOLTAGE_TEMPERATURES: List[float] = get_list_from_config("DEFAULT", "EXPECTED_AUX_VOLTAGE_TEMPERATURES", float)
if (
    not EXPECTED_AUX_VOLTAGES
    or not EXPECTED_AUX_VOLTAGE_TEMPERATURES
    or len(EXPECTED_AUX_VOLTAGES) < 2
    or len(EXPECTED_AUX_VOLTAGES) != len(EXPECTED_AUX_VOLTAGE_TEMPERATURES)
):
    EXPECTED_AUX_VOLTAGES = None
    EXPECTED_AUX_VOLTAGE_TEMPERATURES = None
MAX_AUX_VOLTAGE_DEVIATION_PERCENTS: float = get_float_from_config("DEFAULT", "MAX_AUX_VOLTAGE_DEVIATION_PERCENTS")
INVERT_SMARTSHUNTS: bool = get_bool_from_config("DEFAULT", "INVERT_SMARTSHUNTS")
SMARTSHUNT_ABSENCE_BEHAVIOR: bool = config["DEFAULT"]["SMARTSHUNT_ABSENCE_BEHAVIOR"].lower()
